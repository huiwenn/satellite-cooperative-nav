...
class World(object):
    ...
    def step(self):
        ...
        for agent in self.agents:
            ...
            #agent.actions_hist.append(agent.action.u)
        if self.cache_dists:
...
class SatWorld(object):
    ...
    def dynamics(self, s,u):
        ...
        re= 6378.1363 #km 
        J2= 1.08262668e-3
        i=7*(math.pi/180) #1.7082196041 #rad 98 degrees
        ss = ((3*J2*(re**2))/(8*a**2)) * (1+3*math.cos(2*i))
        c =  (1+ss)**.5
        # print('c is '+str(c))
        ...
        s = np.asarray(s)
        u = np.asarray(u)
        #u = np.asarray([u[0]/1000, u[1]/1000])
        # print('s is '+str(s))
        #print('u is '+str(u))
    ...
    def integrate_sat_state(self, p_force):
        ...
        for i,entity in enumerate(self.entities):
            ...
            #dt = 30
            #print('core self dt '+str(self.dt))
            x =x + dsdt*self.dt	
            #print(str(x[0:2]))
